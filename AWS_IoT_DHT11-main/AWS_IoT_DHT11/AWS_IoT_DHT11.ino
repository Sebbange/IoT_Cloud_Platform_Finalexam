...
void setup() {
  Serial.begin(115200); //보드레이트
  while (!Serial);

  dht.begin();
  pinMode(2, OUTPUT); //빗물 감지 센서
...
void getDeviceStatus(char* payload) {
  ...
  if (analogRead(0) < 300) {
    ...
    // Read pump status
  const char* pump = (pump1.getState() == PUMP_ON)? "ON" : "OFF";

  // make payload for the device update topic ($aws/things/MyMKRWiFi1010/shadow/update)
  //sprintf(payload,"{\"state\":{\"reported\":{\"temperature\":\"%0.2f\",\"Rain\":\"%s\",\"PUMP\":\"%s\"}}}",t,rain,pump);
  sprintf(payload,"{\"state\":{\"reported\":{\"humidity\":\"%0.2f\",\"Rain\":\"%s\",\"PUMP\":\"%s\"}}}",h,rain,pump);
}
...
void onMessageReceived(int messageSize) {
  ...
  if (strcmp(pump,"ON")==0) {
    pump1.on();
    sprintf(payload,"{\"state\":{\"reported\":{\"PUMP\":\"%s\"}}}","ON");
    sendMessage(payload);
    
  } else if (strcmp(pump,"OFF")==0) {
    pump1.off();
    sprintf(payload,"{\"state\":{\"reported\":{\"PUMP\":\"%s\"}}}","OFF");
    sendMessage(payload);
  }
