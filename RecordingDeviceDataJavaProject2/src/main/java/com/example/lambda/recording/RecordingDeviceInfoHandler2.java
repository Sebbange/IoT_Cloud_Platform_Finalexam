...
//*****DynamoDB와 AWS Lambda를 이용한 IoT Data 저장 백엔드 구축
//https://kwanulee.github.io/IoTPlatform/dynamodb.html#4
...
public class RecordingDeviceInfoHandler2 implements RequestHandler<Document, String> {
    ...
    private String persistData(Document document) throws ConditionalCheckFailedException {
        ...
        // humidity와 PUMP 값이 이전상태와 동일한 경우 테이블에 저장하지 않고 종료 
        if (document.current.state.reported.humidity.equals(document.previous.state.reported.humidity) && 
                document.current.state.reported.PUMP.equals(document.previous.state.reported.PUMP)) {
                return null;
        }
        ...
        return this.dynamoDb.getTable(DYNAMODB_TABLE_NAME)
                .putItem(new PutItemSpec().withItem(new Item().withPrimaryKey("deviceId", document.device)
                        ...
                        .withString("PUMP", document.current.state.reported.PUMP)
                        .withString("Rain", document.current.state.reported.Rain)
